...
router.post("/onceOnly", (req, res) => {
    ...
    const regImgs = [
        "1732168695876", "1732168831822", "1732169010839", "1732169216932", "1732169292550",
        "1732169578965", "1732170577231", "1732170667091", "1732170681684", "1732170717726",
        "1732170740360", "1732170792531", "1732171192129", "1732181252274", "1732197805218",
        "1732197831269", "1732197999202", "1732198162042", "1732199351648", "1732264735113",
        "1732265067568", "1732281689639"
    ]
...
const cacheImages = async (prompt, completion) => {
    ...
    if (completion) {
        for (const imgValue of Object.values(completion)) {
            ...
            const key = (typeof prompt !== "string" ? JSON.stringify(prompt) : prompt)
            ...
            .then(async() => {
                ...
                console.log("%cimage downloaded and cached", "color:orange", localUrl)
            })
       ...
       const rows = await request(db, "SELECT * FROM images")
        ...
        res.status(200).json({
            ...
            images: rows.map(row => {
                ...
                return {
                    ...row,
                    url: "http://localhost:3456/images/"+row.path
                }
            }),
        ...
        console.log(err)
        res.status(401).json({type:"image", message: "get generated images failed", error: err})
    } finally {
        db.close()
    }
})
...
router.post("/", (req, res) => {
    if (! req.body || !req.body.request) {
        return res.status(400).json({message:"post dall-e failed!", error: "request is required"})
    }
    console.log("POST DALL-E request received", req.body.request)
    requestDavinci(req.body.request,
        async (choices) => {
        ...
        (err) => res.status(400).json({message:"post dall-e failed!", error: err})
    )
})
